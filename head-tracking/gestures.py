    ...
    def process_data(self) -> None:
        ...
        while self.running:
            if time.time() - start_time > self.detection_timeout:
                log.warning(f"{Colors.YELLOW}??  Detection timeout reached. No gesture detected.{Colors.RESET}")
                self.running = False
                break
    ...
    def start_detection(self) -> None:
        ...
        try:
            ...
            if data_thread.is_alive():
                log.warning(f"{Colors.YELLOW}??  Timeout reached. Stopping detection.{Colors.RESET}")
                self.running = False
